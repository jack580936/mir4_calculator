        ...
        <div id="equipment_num" class="equipment_num mb-3 col-12 col-xl-4 ">
          ...
          <template v-for="item in list" class="material">
            <div class="input-group input-group-sm">
              <img :src="getImageUrl(`${item.key}`)" :class="`input-group img-thumbnail Legendary`" alt="...">
          ...
          <div v-for="strategy in result_data_for_display" class="list-group list-group-flush col-12 col-md-4">
            ...
            <template v-for="material_name in strategy.method">
              <div class="list-group list-group-sm answer-img">
                <img v-if="material_name.value>0 && !['dark_steel', 'powder', 'money'].includes(material_name.name)" :src="getImageUrl(`${material_name.name}`)" :class="`list-group-item img-thumbnail ${strategy.class}`" alt="...">
                <img v-else-if="material_name.value>0 && ['dark_steel', 'powder', 'money'].includes(material_name.name)" :src="getImageUrl(`${material_name.name}`)" :class="`list-group-item img-thumbnail Uncommon`" alt="...">
        ...
        <div class="charge list-group list-group-flush col-12">
          ...
          <template v-for="(value,key) in {'dark_steel': '黑鐵','money':'銅幣'}">
            <div class="list-group create">
              <img v-if="manufacture_fee()[key]>0" :src="getImageUrl(`${key}`)" :class="`list-group-item img-thumbnail Uncommon`" alt="...">
  ...
  computed: {
    result_data_for_display() {
      return [{method: this.total_needed('Legendary'), type_name: "傳說",class:'Legendary'},
        {method: this.total_needed('Epic'), type_name: "英雄 ➙ 傳說",class:'Epic'},
        {method: this.total_needed('Rare'), type_name: "稀有 ➙ 傳說",class:'Rare'}]
    },
    init_inventory() {
