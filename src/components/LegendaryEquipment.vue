      ...
      <div id="equipment" class="col-12 col-xl-6">
        <div id="equipment_num" class="equipment_num mb-3 col-12 col-xl-4 ">
          ...
          <template v-for="item in list" class="material">
            <div class="input-group input-group-sm">
              <img :src="`${item.key}.png`" :class="`input-group img-thumbnail Legendary`" alt="...">
        ...
        <template v-for="material_category in init_inventory">
          <div class="inventory mb-3 col-12 col-sm-6 col-xl-6 col-xxl-4">
            <template v-for="(material,index) in material_category" :key="index">
              <div class="input-group input-group-sm">
                <img :src="`${material.class}.png`" :class="`input-group img-thumbnail ${material.level}`"
                     alt>
        ...
        <div class="inventory mb-3 col-12 col-xl-6 col-xxl-4">
          <template v-for="item in currencies" class="material">
            <div class="input-group input-group-sm">
              <img :src="`${item.name}.png`" class="input-group img-thumbnail Uncommon" alt="...">
          ...
          <div v-for="strategy in result_data_for_display" class="list-group list-group-flush col-12 col-md-4">
            ...
            <template v-for="material_name in strategy.method">
              <div class="list-group list-group-sm answer-img">
                <img v-if="material_name.value>0 && !['dark_steel', 'powder', 'money'].includes(material_name.name)" :src="`${material_name.name}.png`" :class="`list-group-item img-thumbnail ${strategy.class}`" alt="...">
                <img v-else-if="material_name.value>0 && ['dark_steel', 'powder', 'money'].includes(material_name.name)" :src="`${material_name.name}.png`" :class="`list-group-item img-thumbnail Uncommon`" alt="...">
        ...
        <div class="charge list-group list-group-flush col-12">
          ...
          <template v-for="(value,key) in {'dark_steel': '黑鐵','money':'銅幣'}">
            <div class="list-group create">
              <img v-if="manufacture_fee()[key]>0" :src="`${key}.png`" :class="`list-group-item img-thumbnail Uncommon`" alt="...">
...
export default {
  data() {
    return {
      ...
      list: [
        {key: 'weapon', value: null, name: '傳說武器'},
        {key: 'armor', value: null, name: '傳說防具'},
        {key: 'accessories', value: null, name: '傳說飾品'},
        {key: 'secondary_weapon', value: null, name: '傳說副武器'},
        {key: 'earrings', value: null, name: '傳說耳環'},
      ],
      inventory_num: [],
      total_cost: {},
      cost: {},
  ...
  computed: {
    result_data_for_display() {
      return [{method: this.total_needed('Legendary'), type_name: "傳說",class:'Legendary'},
        {method: this.total_needed('Epic'), type_name: "英雄",class:'Epic'},
  ...
  methods: {
    ...
    inventories() {
      this.name_mapping.forEach(item => {
        ...
        [{name: 'Legendary', show_name: '傳說'},
          ...
          {name: 'Rare', show_name: '稀有'}].forEach(level => {
    ...
    subtractInventory(classification, needed, total_inventory) {
      ...
      for (const key in needed) {
        const LegendaryValue= total_inventory[`Legendary_${key}`];
        ...
        let value;
        result[key] = (value = (needed[key] - LegendaryValue * weight[0] - EpicValue * weight[1] - RareValue * weight[2])) > 0 ? value : null
      }
      return result;
    },
    ...
    material_upgrade_cost(classification, total_cost) {
      ...
      if (classification === 'Legendary') {
        Object.assign(this.display_cost, {'Legendary': {}})
      } else if (classification === 'Epic') {
        for (let [_, v] of Object.entries(total_cost['Legendary'])) {
          darkSteel += v * 25000
          powder += v * 125
          money += v * 100000
        }
